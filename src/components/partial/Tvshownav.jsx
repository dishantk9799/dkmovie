...
function Tvshownav({category}) {
  ...
  return (
    ...
    <div className='z-50 flex flex-col fixed w-full h-[10%] bg-gradient-to-r from-[#0d342b] to-[#0c0c0c] border-b-[2px] border-zinc-400 sm:border-none'>
      <div className='w-full h-full px-4 py-2 flex items-center justify-evenly text-zinc-400 gap-4 sm:gap-40'>
        <div className='trend flex justify-center items-center gap-1 '><span className='mt-1 sm:text-3xl'><FaTv /></span><h1 className='text-[5vw] font-semibold sm:text-[2vw]'>TV</h1></div>
        ...
        <div className='hidden sm:flex sm:justify-center sm:items-center sm:gap-5'>
            <Dropdown title="Category" option={["ON_THE_AIR","POPULAR","TOP_RATED","AIRING_TODAY"]} onchange={(e)=> category(e.target.value)}/>
      ...
      <div className='absolute w-1/2 max-h-[40vw] overflow-auto bg-zinc-800 top-[90%] left-[23%] z-10 rounded-md font-medium text-zinc-100 sm:max-h-[20vw] sm:left-[19.3%] '>
      ...
      <div className={`${filter ? "hidden" : "flex"}   absolute w1/2 max-h-[40vw] top-[110%] left-[66%] z-0  overflow-auto gap-2 flex-col justify-start items-centersm:hidden`}>
      <Dropdown title="Category" option={["ON_THE_AIR","POPULAR","TOP_RATED","AIRING_TODAY"]} onchange={(e)=> category(e.target.value)}/>
      </div>
    </div>
    </>
