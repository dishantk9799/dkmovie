...
function Moviedetails() {
  document.title = 'Dkmovie - Movies Details';
  ...
  return info ? (
    <>
      <div style={{
        ...
        <div className='mt-2 gap-1 w-full flex items-start justify-start border-b-[2px] pb-2 border-[#24cfa6] sm:border-white sm:mt-4 sm:gap-5'>
          ...
          <div className='toprightpart w-[60%] h-auto flex flex-col justify-start items-start sm:w-[81%]'>
            <div className='flex justify-star  items-start w-full h-auto'>
              <h1 className='text-[6vw] leading-snug overflow-hidden text-ellipsis line-clamp-2  font-semibold sm:text-[3vw] sm:line-clamp-1'>{info.detail.name || info.detail.title || info.detail.original_name || info.detail.original_title} <span className='text-sm text-zinc-200 sm:text-2xl'>({info.detail.release_date.split("-")[0]})</span></h1>
            </div>
            <div className='score text-zinc-100 gap-3 flex justify-star items-center w-full h-auto sm:gap-8'>
              ...
              <small className='hidden sm:block sm:text-lg sm:mt-1'>{info.detail.release_date}</small>
              <h1 className='hidden sm:block sm:text-lg sm:mt-1'>{info.detail.genres.map((g) => g.name).join(",")}</h1>
              <p className='hidden sm:block sm:text-lg sm:mt-1'>{info.detail.runtime}min</p>
            </div>
            <div className='time text-zinc-100 gap-2 flex justify-star items-end w-full h-auto sm:hidden'>
              <small className='text-sm font-medium sm:hidden'>{info.detail.release_date}</small>
              <small className='text-sm font-medium'>{info.detail.runtime}min</small>
            </div>
        ...
        {/* bottom overview */}
        <div className=' gap-1 mt-5 w-full flex flex-col items-start justify-start pb-2 border-b-[2px] border-[#24cfa6] sm:hidden'>
          ...
          <div className='cardmainnav w-full h-full gap-2 overflow-x-auto flex-wrap flex-col flex justify-start items-start sm:gap-11 '>
            {info.recommendations.length > 0 ? (
              info.recommendations.map((item, index) => (
                <Link to={`/${item.media_type || 'movie'}/details/${item.id}`} key={index} className='cardnav w-40 h-52 bg-zinc-800 mt-5 overflow-x-auto rounded-md shadow-md flex flex-col justify-start items-start p-2'>
                  ...
                  <div className='w-full h-1/2'>
                    ...
                    <p className='text-white text-[3vw] sm:text-[.8vw]'>{item.overview.slice(0, 40)}<Link to={`/${item.media_type || 'movie'}/details/${item.id}`} className='text-blue-400'>...more</Link></p>
                  </div>
                </Link>
              ))
